...
system_file = './fsystem_sub_v3.txt'
    ...
    def _find_tree_images_by_ID(self):
        ...
        for i in lis:
            ...
            if search_id.upper() == self.ID:
                ...
                if rn.upper() == 'R6':
                    mean_spectral_file = os.path.join('./R6_data/r6', img_name, 'kmeans', 'mean_average.txt')
                    cls_file = os.path.join('./R6_data/r6', img_name, 'kmeans', 'cls.tif')
...
def plot_tree_data(tree):
    ...
    for tree_img in tree.tree_images:
        ...
        rn = tree_img.rn
        month = month_label[int(rn.upper()[-1]) - 1]
        color_index = int(rn[-1])
        status = tree_img.status
        status_name += month + '({})->'.format(status)
...
def plot_tree(tree):
    ...
    plot_mean_spectral_round(g_n, s_n, spectral, './trees_6')
    # plot all
    # if healthy != []:
...
def plot_species_rn(specie_dict_ls, species_list_rn):
    ...
    for idx, specie in enumerate(specie_dict_ls):
        for idx_rn, species in enumerate(species_list_rn):
            ...
            plt.plot(bands, mean_spectral, color=colors[idx_rn, :]/255., label=month_label[idx_rn])
        plt.xlabel('Wavelength(nm)')
        plt.ylabel('Reflectance')
        plt.title(specie)
        plt.xlim(380, 1020)
        ...
        plt.grid()
        plt.legend(bbox_to_anchor=(1.04, 1), loc="upper left")
        new_figure_file = os.path.join('./trees_6', 'specie', specie + '.png')
        plt.savefig(new_figure_file, bbox_inches="tight", dpi=300)
        plt.close()
    # print(index_species_rn['NDVI'][0][0])
    # print(count_temp)
    np.save('./trees_6/specie/mean_spectral_species', out)
...
def plot_species_rn_indexes(specie_dict_ls, species_list_rn):
    ...
    count_temp = 0
    outB_fs = open('./trees_6/indexes/bands_6.txt', 'w')
    out_fs = open('./trees_6/indexes/indices2_6.txt', 'w')
    for idx, specie in enumerate(specie_dict_ls):
        for idx_rn, species in enumerate(species_list_rn):
            for tree in species[specie]:
                if tree.mean_spectral_without_B_S is not None:
    ...
    specie_dict = np.load('./species_dict.npy', allow_pickle=True)
    specie_dict = specie_dict.tolist()
    month_label = ['Dec', 'Feb', 'Apr', 'Jun', 'Aug', 'Oct']
    for key in index_species_rn.keys():
        ...
        # minor_label = [str(i+1) for i in range(6)] * 19
        minor_label = [month_label[i] for i in range(6)] * 19
        for idx_sp, specie in enumerate(test):
            # major_label.append(str(idx_sp) + '_' + specie_ls[idx_sp][:5])
            major_label.append(specie_dict[specie_ls[idx_sp]])
            major_ticks.append(idx_sp * 7)
            for idx_rn, rn in enumerate(specie):
                # print(idx_rn)
                out = idx_sp * 7 + idx_rn + 1
        ...
        ax.set_xticklabels(major_label, rotation=90, fontsize=12)
        ax.set_xticklabels(minor_label, minor=True, rotation=90)
        plt.xlim((0, 133))
        plt.xlabel('Species_Round')
        plt.ylabel('Index_Value')
        plt.title(key)
        plt.savefig(os.path.join('./trees_6/indexes', key+'.png'),
...
if __name__ == '__main__22':
    ...
    plot_species_rn(specie_dict_ls, specie)
...
# generate HDF files
if __name__ == '__main__generateHDF':
    tree_ls = [Tree(i) for i in np.load('./tree_id.npy')]
    specie_dict_ls = {i: [] for i in np.load('./species.npy')}
    [specie_dict_ls[i.specie].append(i) for i in tree_ls]
    # print(specie_dict_ls)
    ...
    fs = open('./fsystem_sub_v3.txt', 'r').readlines()
    spectral_dir = '../../../spectral_data'
    for r_idex, r in enumerate(specie):
        print(r_idex)
        h5f = h5py.File('./nn_test/dataset/train_' + str(r_idex+6) + '.h5', 'w')
        ...
        for label_idx, spc in enumerate(r.keys()):
            for tree in r[spc]:
                ...
                file_path = ''
                cls_file = os.path.join('./R6_data/r6', tree.image_name, 'kmeans', 'cls.tif')
                for temp_path in fs:
...
def dlm(flag, round_num, specie_dict_ls):
    # flag = 1234
    # round_num = 5
    cls_file_dir = './R{}_data/r{}'.format(round_num, round_num) if (round_num == 5 or round_num == 6) else './all_data'
    # cls_file_dir = './R6_data/r6' if round_num == 6 else
        ...
        for j in specie_dict_ls[i]:
            for k in j.tree_images:
                if k.rn.upper() == ('R' + str(round_num)):
                    r1.append(k)
    print(len(r1))
    # fs = open('./system_sub_v3.txt', 'r').readlines()
    fs = open('./fsystem_sub_v3.txt', 'r').readlines()
